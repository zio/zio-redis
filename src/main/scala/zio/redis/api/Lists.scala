...
trait Lists {
  final val brPopLPush =
    RedisCommand("BRPOPLPUSH", Tuple3(StringInput, StringInput, DurationSecondsInput), StringOutput)
  final val lIndex     = RedisCommand("LINDEX", Tuple2(StringInput, LongInput), StringOutput)
  final val lLen       = RedisCommand("LLEN", StringInput, LongOutput)
  final val lPop       = RedisCommand("LPOP", StringInput, StringOutput)
  ...
  final val rPop       = RedisCommand("RPOP", StringInput, StringOutput)
  final val rPopLPush  = RedisCommand("RPOPLPUSH", Tuple2(StringInput, StringInput), StringOutput)
