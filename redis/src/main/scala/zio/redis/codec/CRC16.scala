 ...
 */
private[redis] object CRC16 {
  private final val Lookup =
    Array(0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50a5, 0x60c6, 0x70e7, 0x8108, 0x9129, 0xa14a, 0xb16b, 0xc18c,
  ...
  def get(bytes: Chunk[Byte]): Int =
    bytes.foldLeft(0) { case (crc, b) => (crc << 8) ^ Lookup(((crc >> 8) ^ b) & 0xff) } & 0xffff
