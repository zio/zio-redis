...
trait Hashes {
   ...
   */
  final def hDel[K: Schema, F: Schema](key: K, field: F, fields: F*): ZIO[RedisEnv, RedisError, Long] = {
   ...
   */
  final def hExists[K: Schema, F: Schema](key: K, field: F): ZIO[RedisEnv, RedisError, Boolean] = {
  ...
  final def hGetAll[K: Schema](key: K): ResultBuilder2[Map] = new ResultBuilder2[Map] {
    def returning[F: Schema, V: Schema]: ZIO[RedisEnv, RedisError, Map[F, V]] = {
      val command =
   ...
   */
  final def hIncrBy[K: Schema, F: Schema](key: K, field: F, increment: Long): ZIO[RedisEnv, RedisError, Long] = {
  ...
  final def hKeys[K: Schema](key: K): ResultBuilder1[Chunk] =
    new ResultBuilder1[Chunk] {
      def returning[F: Schema]: ZIO[RedisEnv, RedisError, Chunk[F]] =
   ...
   *   number of fields.
   */
  final def hLen[K: Schema](key: K): ZIO[RedisEnv, RedisError, Long] = {
  ...
  final def hScan[K: Schema](
    ...
    new ResultBuilder2[({ type lambda[x, y] = (Long, Chunk[(x, y)]) })#lambda] {
      def returning[F: Schema, V: Schema]: ZIO[RedisEnv, RedisError, (Long, Chunk[(F, V)])] = {
        val command = RedisCommand(
          HScan,
   ...
   */
  final def hStrLen[K: Schema, F: Schema](key: K, field: F): ZIO[RedisEnv, RedisError, Long] = {
