...
trait SortedSets {
  ...
  final def bzPopMax[K: Schema](
    ...
    new ResultBuilder1[({ type lambda[x] = Option[(K, MemberScore[x])] })#lambda] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, Option[(K, MemberScore[M])]] = {
        val memberScoreOutput =
          Tuple3Output(ArbitraryOutput[K](), ArbitraryOutput[M](), DoubleOutput).map { case (k, m, s) =>
            (k, MemberScore(s, m))
          }
        val command = RedisCommand(BzPopMax, Tuple2(NonEmptyList(ArbitraryInput[K]()), DurationSecondsInput), OptionalOutput(memberScoreOutput))
  ...
  final def bzPopMin[K: Schema](
    ...
    new ResultBuilder1[({ type lambda[x] = Option[(K, MemberScore[x])] })#lambda] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, Option[(K, MemberScore[M])]] = {
        ...
        val command = RedisCommand(BzPopMin, Tuple2(NonEmptyList(ArbitraryInput[K]()), DurationSecondsInput), OptionalOutput(memberScoreOutput))
        command.run(((key, keys.toList), timeout))
      }
    }
  ...
  ): ZIO[RedisEnvironment, RedisError, Long] = {
    val command = RedisCommand(ZAdd, Tuple4(
      ArbitraryInput[K](),
  ...
  final def zAddWithIncr[K: Schema, M: Schema](key: K, update: Option[Update] = None, change: Option[Changed] = None)(
    ...
    memberScore: MemberScore[M],
    memberScores: MemberScore[M]*
  ): ZIO[RedisEnvironment, RedisError, Option[Double]] = {
   ...
   */
  final def zCard[K: Schema](key: K): ZIO[RedisEnvironment, RedisError, Long] = {
   ...
   */
  final def zCount[K: Schema](key: K, range: Range): ZIO[RedisEnvironment, RedisError, Long] = {
  ...
  final def zDiff[K: Schema](
    ...
    new ResultBuilder1[Chunk] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, Chunk[M]] = {
        val command =
          RedisCommand(ZDiff, Tuple2(
            LongInput,
  ...
  final def zDiffWithScores[K: Schema](
    ...
    new ResultBuilder1[MemberScores] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, Chunk[MemberScore[M]]] = {
        val command =
          RedisCommand(ZDiff, Tuple3(
            LongInput,
            NonEmptyList(ArbitraryInput[K]()),
  ...
  final def zDiffStore[DK: Schema, K: Schema](
    ...
    val command =
      RedisCommand(ZDiffStore, Tuple3(
        ArbitraryInput[DK](),
        LongInput,
        NonEmptyList(ArbitraryInput[K]())
      ), LongOutput)
  ...
  final def zIncrBy[K: Schema, M: Schema](
    ...
    increment: Long,
    member: M
  ): ZIO[RedisEnvironment, RedisError, Double] = {
    val command =
      RedisCommand(ZIncrBy, Tuple3(ArbitraryInput[K](), LongInput, ArbitraryInput[M]()), DoubleOutput)
  ...
  final def zInter[K: Schema](inputKeysNum: Long, key: K, keys: K*)(
    ...
    new ResultBuilder1[Chunk] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, Chunk[M]] = {
        val command = RedisCommand(ZInter, Tuple4(
          LongInput,
          NonEmptyList(ArbitraryInput[K]()),
  ...
  final def zInterWithScores[K: Schema](inputKeysNum: Long, key: K, keys: K*)(
    ...
    new ResultBuilder1[MemberScores] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, Chunk[MemberScore[M]]] = {
        val command = RedisCommand(ZInter, Tuple5(
          LongInput,
          NonEmptyList(ArbitraryInput[K]()),
          OptionalInput(AggregateInput),
          OptionalInput(WeightsInput),
  ...
  final def zInterStore[DK: Schema, K: Schema](destination: DK, inputKeysNum: Long, key: K, keys: K*)(
    ...
    val command = RedisCommand(ZInterStore, Tuple5(
      ArbitraryInput[DK](),
      LongInput,
      NonEmptyList(ArbitraryInput[K]()),
      OptionalInput(AggregateInput),
   ...
   *   The number of elements in the specified score range.
   */
  final def zLexCount[K: Schema](key: K, lexRange: LexRange): ZIO[RedisEnvironment, RedisError, Long] = {
    val command = RedisCommand(ZLexCount, Tuple3(ArbitraryInput[K](), ArbitraryInput[String](), ArbitraryInput[String]()), LongOutput)
  ...
  final def zPopMax[K: Schema](
    ...
    new ResultBuilder1[MemberScores] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, Chunk[MemberScore[M]]] = {
        val command = RedisCommand(ZPopMax, Tuple2(ArbitraryInput[K](), OptionalInput(LongInput)), ChunkTuple2Output(ArbitraryOutput[M](), DoubleOutput)
          .map(_.map { case (m, s) => MemberScore(s, m) }))
  ...
  final def zPopMin[K: Schema](
    ...
    new ResultBuilder1[MemberScores] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, Chunk[MemberScore[M]]] = {
        val command = RedisCommand(ZPopMin, Tuple2(ArbitraryInput[K](), OptionalInput(LongInput)), ChunkTuple2Output(ArbitraryOutput[M](), DoubleOutput)
          .map(_.map { case (m, s) => MemberScore(s, m) }))
        command.run((key, count))
      }
    }
  ...
  final def zRange[K: Schema](key: K, range: Range): ResultBuilder1[Chunk] =
    new ResultBuilder1[Chunk] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, Chunk[M]] = {
        val command = RedisCommand(ZRange, Tuple2(ArbitraryInput[K](), RangeInput), ChunkOutput(ArbitraryOutput[M]()))
        command.run((key, range))
      }
    }
  ...
  final def zRangeWithScores[K: Schema](
    ...
    new ResultBuilder1[MemberScores] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, Chunk[MemberScore[M]]] = {
        val command = RedisCommand(ZRange, Tuple3(ArbitraryInput[K](), RangeInput, ArbitraryInput[String]()), ChunkTuple2Output(ArbitraryOutput[M](), DoubleOutput)
          .map(_.map { case (m, s) => MemberScore(s, m) }))
  ...
  final def zRangeByLex[K: Schema](
    ...
    new ResultBuilder1[Chunk] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, Chunk[M]] = {
        val command = RedisCommand(ZRangeByLex, Tuple4(ArbitraryInput[K](), ArbitraryInput[String](), ArbitraryInput[String](), OptionalInput(LimitInput)), ChunkOutput(ArbitraryOutput[M]()))
  ...
  final def zRangeByScore[K: Schema](
    ...
    new ResultBuilder1[Chunk] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, Chunk[M]] = {
        val command = RedisCommand(ZRangeByScore, Tuple4(ArbitraryInput[K](), ArbitraryInput[String](), ArbitraryInput[String](), OptionalInput(LimitInput)), ChunkOutput(ArbitraryOutput[M]()))
  ...
  final def zRangeByScoreWithScores[K: Schema](
    ...
    new ResultBuilder1[MemberScores] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, Chunk[MemberScore[M]]] = {
        val command = RedisCommand(ZRangeByScore, Tuple5(
          ArbitraryInput[K](),
          ArbitraryInput[String](),
          ArbitraryInput[String](),
          ArbitraryInput[String](),
   ...
   */
  final def zRank[K: Schema, M: Schema](key: K, member: M): ZIO[RedisEnvironment, RedisError, Option[Long]] = {
    val command =
      RedisCommand(ZRank, Tuple2(ArbitraryInput[K](), ArbitraryInput[M]()), OptionalOutput(LongOutput))
  ...
  final def zRem[K: Schema, M: Schema](
    ...
    val command =
      RedisCommand(ZRem, Tuple2(ArbitraryInput[K](), NonEmptyList(ArbitraryInput[M]())), LongOutput)
   ...
   */
  final def zRemRangeByLex[K: Schema](key: K, lexRange: LexRange): ZIO[RedisEnvironment, RedisError, Long] = {
    val command = RedisCommand(ZRemRangeByLex, Tuple3(ArbitraryInput[K](), ArbitraryInput[String](), ArbitraryInput[String]()), LongOutput)
    command.run((key, lexRange.min.stringify, lexRange.max.stringify))
  }
   ...
   *   The number of elements removed.
   */
  final def zRemRangeByRank[K: Schema](key: K, range: Range): ZIO[RedisEnvironment, RedisError, Long] = {
   ...
   *   The number of elements removed.
   */
  final def zRemRangeByScore[K: Schema](key: K, scoreRange: ScoreRange): ZIO[RedisEnvironment, RedisError, Long] = {
    val command = RedisCommand(ZRemRangeByScore, Tuple3(ArbitraryInput[K](), ArbitraryInput[String](), ArbitraryInput[String]()), LongOutput)
  ...
  final def zRevRange[K: Schema](key: K, range: Range): ResultBuilder1[Chunk] =
    new ResultBuilder1[Chunk] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, Chunk[M]] = {
        val command = RedisCommand(ZRevRange, Tuple2(ArbitraryInput[K](), RangeInput), ChunkOutput(ArbitraryOutput[M]()))
        command.run((key, range))
      }
    }
  ...
  final def zRevRangeWithScores[K: Schema](
    ...
    new ResultBuilder1[MemberScores] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, Chunk[MemberScore[M]]] = {
        val command = RedisCommand(ZRevRange, Tuple3(ArbitraryInput[K](), RangeInput, ArbitraryInput[String]()), ChunkTuple2Output(ArbitraryOutput[M](), DoubleOutput)
          .map(_.map { case (m, s) => MemberScore(s, m) }))
        command.run((key, range, WithScores.stringify))
      }
    }
  ...
  final def zRevRangeByLex[K: Schema](
    ...
    new ResultBuilder1[Chunk] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, Chunk[M]] = {
        val command = RedisCommand(ZRevRangeByLex, Tuple4(ArbitraryInput[K](), ArbitraryInput[String](), ArbitraryInput[String](), OptionalInput(LimitInput)), ChunkOutput(ArbitraryOutput[M]()))
  ...
  final def zRevRangeByScore[K: Schema](
    ...
    new ResultBuilder1[Chunk] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, Chunk[M]] = {
        val command = RedisCommand(ZRevRangeByScore, Tuple4(
          ArbitraryInput[K](),
          ArbitraryInput[String](),
          ArbitraryInput[String](),
          OptionalInput(LimitInput)
  ...
  final def zRevRangeByScoreWithScores[K: Schema](
    ...
    new ResultBuilder1[MemberScores] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, Chunk[MemberScore[M]]] = {
        val command = RedisCommand(ZRevRangeByScore, Tuple5(
          ArbitraryInput[K](),
          ArbitraryInput[String](),
          ArbitraryInput[String](),
          ArbitraryInput[String](),
   ...
   *   The rank of member.
   */
  final def zRevRank[K: Schema, M: Schema](key: K, member: M): ZIO[RedisEnvironment, RedisError, Option[Long]] = {
  ...
  final def zScan[K: Schema](
    ...
    new ResultBuilder1[({ type lambda[x] = (Long, MemberScores[x]) })#lambda] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, (Long, Chunk[MemberScore[M]])] = {
        val memberScoresOutput =
          ChunkTuple2Output(ArbitraryOutput[M](), DoubleOutput).map(_.map { case (m, s) => MemberScore(s, m) })
        val command = RedisCommand(ZScan, Tuple4(ArbitraryInput[K](), LongInput, OptionalInput(PatternInput), OptionalInput(CountInput)), Tuple2Output(MultiStringOutput.map(_.toLong), memberScoresOutput))
   ...
   */
  final def zScore[K: Schema, M: Schema](key: K, member: M): ZIO[RedisEnvironment, RedisError, Option[Double]] = {
  ...
  final def zUnionStore[DK: Schema, K: Schema](destination: DK, inputKeysNum: Long, key: K, keys: K*)(
    ...
    val command = RedisCommand(ZUnionStore, Tuple5(
      ArbitraryInput[DK](),
      LongInput,
      NonEmptyList(ArbitraryInput[K]()),
      OptionalInput(WeightsInput),
   ...
   */
  final def zMScore[K: Schema](key: K, keys: K*): ZIO[RedisEnvironment, RedisError, Chunk[Option[Double]]] = {
  ...
  final def zRandMember[K: Schema](key: K): ResultBuilder1[Option] =
    new ResultBuilder1[Option] {
      def returning[R: Schema]: ZIO[RedisEnvironment, RedisError, Option[R]] =
  ...
  final def zRandMember[K: Schema](
    ...
    new ResultBuilder1[Chunk] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, Chunk[M]] = {
        val command = RedisCommand(ZRandMember, Tuple2(ArbitraryInput[K](), LongInput), ZRandMemberOutput(ArbitraryOutput[M]()))
        command.run((key, count))
      }
    }
  ...
  final def zRandMemberWithScores[K: Schema](
    ...
    new ResultBuilder1[MemberScores] {
      def returning[M: Schema]: ZIO[RedisEnvironment, RedisError, Chunk[MemberScore[M]]] = {
        val command = RedisCommand(ZRandMember, Tuple3(ArbitraryInput[K](), LongInput, ArbitraryInput[String]()), ZRandMemberTuple2Output(ArbitraryOutput[M](), DoubleOutput)
          .map(_.map { case (m, s) => MemberScore(s, m) }))
