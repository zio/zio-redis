...
trait Lists {
   ...
   * This is the blocking variant of [[zio.redis.api.Lists#rPopLPush]]
   *
   * @param source the key identifier of the source list
   * @param destination the key identifier of the target list
   * @param timeout the maximum time to wait for an element to be available. A timeout of zero can be used to block indefinitely
   * @return the element being popped from source and pushed to destination. If timeout is reached, an empty reply is returned
   */
   ...
   * @param index the requested index. It is zero-based, so 0 means the first element, 1 the second element and so on. Negative indices can be used to designate elements starting at the tail of the list
   * @return a ZIO value containing the requested element, or empty if the index is out of range
   */
   ...
   *
   * @param key the key identifier
   * @param range the range of elements to retrieve. The range should be zero-based, with 0 being the first element of the list (the head of the list), 1 being the next element and so on
   ...
   *
   * @param key the key identifier. Non-existing keys are treated like empty lists, so when key does not exist, the command will always return 0
   ...
   *
   * @param key the key identifier
   * @param index the requested index. The index is zero-based, so 0 means the first element, 1 the second element and so on.
   ...
   *
   * @param key the key identifier
   * @param range the range of elements to trim. The range should be zero-based, with 0 being the first element of the list (the head of the list), 1 being the next element and so on
   * @return the Unit value
   */
   ...
   * @return the element being popped and pushed. If source does not exist, empty is returned and no operation is performed.
   */
  ...
  /** Removes and gets the first element in a list, or block until one is available. An element is popped from the head of the first list that is non-empty, with the given keys being checked in the order that they are given
   *
   * @param key the key identifier of the first list to be checked
   * @param keys the key identifiers of the rest of the lists
   * @param timeout the maximum time to wait until an element is available. A timeout of zero can be used to block indefinitely
   * @return a tuple with the first element being the name of the key where an element was popped and the second element being the value of the popped element.
   *         Empty is returned when no element could be popped and the timeout expired
   */
