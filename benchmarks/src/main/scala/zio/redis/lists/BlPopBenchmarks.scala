...
import zio.redis.{Benchmark, blPop, rPush}
...
@Fork(2)
class BlPopBenchmarks extends Benchmark {
  @Param(Array("500"))
  var count: Int = _
  ...
  @Benchmark
  def zio(): Unit = zioUnsafeRun(ZIO.foreach_(items)(_ => blPop(key)(1.second).returning[String]))
}
